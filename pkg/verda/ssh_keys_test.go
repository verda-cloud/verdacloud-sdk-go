...
func TestSSHKeyService_DeprecatedMethods(t *testing.T) {
	mockServer := testutil.NewMockServer()
	defer mockServer.Close()
	...
	t.Run("deprecated Create method", func(t *testing.T) {
		...
		req := CreateSSHKeyRequest{
			...
			PublicKey: "ssh-rsa AAAAB3NzaC1yc2ETEST...",
		}
	...
	t.Run("deprecated Delete method", func(t *testing.T) {
		...
		// Create a key first
		createReq := CreateSSHKeyRequest{
			Name:      "Key for Deprecated Delete",
			PublicKey: "ssh-rsa AAAAB3NzaC1yc2EDEL...",
		}

		key, err := client.SSHKeys.Create(ctx, createReq)
		if err != nil {
			t.Fatalf("failed to create key: %v", err)
		}

		// Delete using deprecated method
		err = client.SSHKeys.Delete(ctx, key.ID)
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	})
