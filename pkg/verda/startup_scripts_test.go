...
func TestStartupScriptService_DeprecatedMethods(t *testing.T) {
	mockServer := testutil.NewMockServer()
	defer mockServer.Close()
	...
	t.Run("deprecated GetByID method", func(t *testing.T) {
		...
		script, err := client.StartupScripts.GetByID(ctx, scriptID)
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	...
	t.Run("deprecated Delete method", func(t *testing.T) {
		...
		createReq := CreateStartupScriptRequest{
			Name:   "Script for Deprecated Delete",
			Script: "#!/bin/bash\necho 'delete me'",
		}

		script, err := client.StartupScripts.Create(ctx, createReq)
		if err != nil {
			t.Fatalf("failed to create script: %v", err)
		}

		// Delete using deprecated method
		err = client.StartupScripts.Delete(ctx, script.ID)
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
	})
